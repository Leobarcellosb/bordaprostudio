...
-- Add machine settings to profiles
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS machine_format text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS machine_hoop_size text;

-- Update search_designs to support machine_format filtering
...
RETURNS TABLE(
  ...
  category_name text, relevance integer, total_count bigint
)
...
BEGIN
  ...
  counted AS (
    SELECT count(*) AS cnt FROM filtered WHERE filtered.rel_score > 0 OR normalized_term = ''
  )
  SELECT
  ...
  ORDER BY
    CASE WHEN p_sort = 'recent' THEN 0 END,
