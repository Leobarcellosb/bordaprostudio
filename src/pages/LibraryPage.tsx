  ...
  useEffect(() => {
    const fetchData = async () => {
      const [{ data: kitsData }, { data: catsData }, { data: tagsData }, { data: ktData }, { data: kfData }] = await Promise.all([
        supabase.from("kits").select("*").eq("is_published", true).order("created_at", { ascending: false }),
        supabase.from("categories").select("*").order("name"),
        supabase.from("tags").select("*").order("name"),
        supabase.from("kit_tags").select("*, tags(*)"),
        supabase.from("kit_files").select("kit_id, file_format"),
      ]);
      setKits(kitsData || []);
      ...
      (ktData || []).forEach((kt: any) => {
        if (!tagMap[kt.kit_id]) tagMap[kt.kit_id] = [];
        tagMap[kt.kit_id].push(kt.tags);
      });
      setKitTags(tagMap);

      const fileMap: Record<string, any[]> = {};
      (kfData || []).forEach((kf: any) => {
        if (!fileMap[kf.kit_id]) fileMap[kf.kit_id] = [];
        if (!fileMap[kf.kit_id].includes(kf.file_format)) fileMap[kf.kit_id].push(kf.file_format);
      });
      setKitFiles(fileMap);
    };
    fetchData();
              ...
              <Card key={kit.id} className="cursor-pointer hover:shadow-lg transition-all hover:-translate-y-0.5" onClick={() => navigate(`/library/${kit.id}`)}>
                ...
                <CardContent className="pt-3 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(kitTags[kit.id] || []).slice(0, 3).map((t: any) => (
                      <Badge key={t?.id} variant="secondary" className="text-xs">{t?.name}</Badge>
                    ))}
                    {(kitFiles[kit.id] || []).slice(0, 2).map((f: string) => (
