...
const KitDetail = () => {
  ...
  useEffect(() => {
    const fetchKit = async () => {
      ...
      const [{ data: kitData }, { data: filesData }, { data: tagsData }, { data: ideasData }] = await Promise.all([
        supabase.from("kits").select("*, categories(name)").eq("id", id).single(),
        supabase.from("kit_files").select("*").eq("kit_id", id),
        supabase.from("kit_tags").select("*, tags(*)").eq("kit_id", id),
        supabase.from("product_ideas").select("*").eq("kit_id", id),
      ]);
      setKit(kitData);
      setFiles(filesData || []);
  ...
  const handleDownload = async (file: any) => {
    if (user && id) {
      await supabase.from("downloads").insert({ user_id: user.id, kit_id: id });
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              {kit.categories?.name && (
                <Badge variant="secondary" className="mt-2">{kit.categories.name}</Badge>
              )}
            </div>
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tags.map((t: any) => <Badge key={t?.id} variant="outline">{t?.name}</Badge>)}
              </div>
            )}
        ...
        {productIdeas.length > 0 && (
          <div>
            <h2 className="text-2xl font-serif font-bold mb-4 flex items-center gap-2">
              <ShoppingBag className="h-5 w-5" /> Ideias de Produtos
            </h2>
                ...
                <Card key={idea.id}>
                  <CardContent className="pt-5">
                    ...
                    {idea.suggested_price && (
                      <p className="text-primary font-bold mt-2">R$ {Number(idea.suggested_price).toFixed(2)}</p>
                    )}
                    <Button variant="outline" size="sm" className="mt-3 w-full" onClick={() => navigate(`/sales-generator?kit=${id}&product=${idea.id}`)}>
