...
const TrendInsights = () => {
  ...
  useEffect(() => {
    ...
    const fetchTrends = async () => {
      ...
      if (!allKits) { setLoading(false); return; }
      ...
      // Categorias Populares — fonte de verdade: tabela folders + deriveFoldersForDesign
      const results: Record<string, any[]> = {};
      const counts: Record<string, number> = {};

      for (const folder of folderList.filter((f) => f.is_active)) {
    ...
    fetchTrends();
  }, [folderList, foldersLoading]);

  // Pastas ativas com pelo menos 1 design, ordenadas por volume
  const activeFolders = folderList
    .filter((f) => f.is_active && (trendCounts[f.slug] ?? 0) > 0)
    .sort((a, b) => (trendCounts[b.slug] ?? 0) - (trendCounts[a.slug] ?? 0));
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Categorias Populares — fonte: tabela folders */}
              ...
              {activeFolders.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {activeFolders.map((folder) => {
                    const designs = trendData[folder.slug] || [];
                    return (
                      <Card key={folder.slug} className="border-border/60 p-4">
                        <FolderSection slug={folder.slug} name={folder.name} designs={designs} />
                      </Card>
                    );
                  })}
                </div>
